import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
from datetime import datetime

# --- CONFIGURACIÓN DE PÁGINA ---
st.set_page_config(page_title="Security CRM Enterprise", layout="wide", page_icon="🛡️")

# --- CONEXIÓN ---
conn = st.connection("gsheets", type=GSheetsConnection)

# --- LISTA DE RESPONSABLES HARDCODEADA (PEDIDO DEL CLIENTE) ---
LISTA_RESPONSABLES = ["Equipo General", "Avir", "Asher", "Kamer", "Jesef", "Adan", "Itza", "Kaleb", "Wyatt"]
ESTADOS = ["1. Por contactar", "2. Primer mensaje enviado", "3. Reunión pactada", "4. Reunión realizada", "5. Aceptó donar (Falta definir monto)", "6. Donación Confirmada", "7. Rechazó"]

# --- CAPA DE DATOS SEGURA ---
def load_data():
    try:
        data = conn.read(ttl=0)
        if data is None or data.empty:
            return pd.DataFrame(columns=["id", "nombre", "apellido", "telefono", "rubro", "contexto", "residencia", "grupo_familiar", "monto_sugerido", "estado", "monto_confirmado", "proximos_pasos", "responsable", "fecha_registro"])
        
        # Sanitización y tipos de datos
        for col in ['monto_confirmado', 'monto_sugerido']:
            data[col] = pd.to_numeric(data[col], errors='coerce').fillna(0).astype(float)
        
        data['id'] = data['id'].astype(str)
        return data
    except Exception:
        return pd.DataFrame(columns=["id", "nombre", "apellido", "telefono", "rubro", "contexto", "residencia", "grupo_familiar", "monto_sugerido", "estado", "monto_confirmado", "proximos_pasos", "responsable", "fecha_registro"])

def save_data(dataframe):
    # FRENO DE MANO: No permitir guardar si el DataFrame está sospechosamente vacío
    if len(dataframe) == 0 and 'last_count' in st.session_state and st.session_state.last_count > 1:
        st.error("🚨 BLOQUEO DE SEGURIDAD: Se detectó un posible error de borrado masivo. No se guardó nada.")
        return False
    
    try:
        # Integridad: Solo el estado 6 puede tener dinero confirmado
        dataframe.loc[dataframe['estado'] != "6. Donación Confirmada", 'monto_confirmado'] = 0
        
        # Guardar en GSheets
        dataframe_to_save = dataframe.astype(str)
        conn.update(data=dataframe_to_save)
        st.cache_data.clear()
        st.session_state.last_count = len(dataframe)
        return True
    except Exception as e:
        st.error(f"❌ Error de conexión: {e}")
        return False

# --- GESTIÓN DE ESTADO DE SESIÓN ---
if 'df' not in st.session_state or st.sidebar.button("🔄 Refrescar Base"):
    st.session_state.df = load_data()
    st.session_state.last_count = len(st.session_state.df)

df = st.session_state.df

# --- SIDEBAR ---
st.sidebar.title("🛡️ CRM Seguridad")
meta_usd = st.sidebar.number_input("Meta Objetivo (USD)", value=500000.0, step=10000.0)
menu = st.sidebar.radio("Navegación", ["📊 Dashboard", "👥 Pipeline", "🆕 Nuevo Registro"])

# --- VISTA: DASHBOARD ---
if menu == "📊 Dashboard":
    st.title("Estado Crítico de Recaudación")
    
    recaudado = float(df[df['estado'] == "6. Donación Confirmada"]['monto_confirmado'].sum())
    negociacion = float(df[df['estado'].isin(ESTADOS[1:5])]['monto_sugerido'].sum())
    proyectado = recaudado + negociacion
    
    c1, c2, c3 = st.columns(3)
    c1.metric("RECAUDADO REAL", f"USD {recaudado:,.0f}")
    c2.metric("PROYECTADO (Real + Pipeline)", f"USD {proyectado:,.0f}")
    c3.metric("TOTAL CONTACTOS", len(df))

    st.markdown("---")
    
    col_a, col_b = st.columns([1.5, 1])
    with col_a:
        fig = go.Figure(go.Indicator(
            mode = "gauge+number", value = recaudado,
            gauge = {'axis': {'range': [None, meta_usd]}, 'bar': {'color': "#2ecc71"}},
            title = {'text': "Avance hacia la Meta"}
        ))
        st.plotly_chart(fig, use_container_width=True)
    
    with col_b:
        st.subheader("USD por Responsable")
        resp_money = df[df['estado'] == "6. Donación Confirmada"].groupby('responsable')['monto_confirmado'].sum().sort_values().reset_index()
        if not resp_money.empty:
            st.plotly_chart(px.bar(resp_money, x='monto_confirmado', y='responsable', orientation='h', color_discrete_sequence=['#3498db']), use_container_width=True)
        else:
            st.info("Aún no hay USD confirmados.")

# --- VISTA: PIPELINE ---
elif menu == "👥 Pipeline":
    st.title("Gestión de Seguimiento")
    busqueda = st.text_input("🔍 Buscar por Nombre, Rubro, Notas, Familia...").lower()
    
    # Lógica de filtrado
    df_f = df[df.apply(lambda r: busqueda in str(r).lower(), axis=1)]

    for idx, row in df_f.iterrows():
        color = "🔵" if "1." in str(row['estado']) else "🟢" if "6." in str(row['estado']) else "🟡"
        with st.expander(f"{color} {row['nombre']} {row['apellido']} | {row['estado']} | {row['responsable']}"):
            
            c_h1, c_h2 = st.columns([2, 1])
            with c_h1:
                st.markdown(f"💰 **Sugerido:** USD {float(row['monto_sugerido']):,.0f} | **Confirmado:** :green[USD {float(row['monto_confirmado']):,.0f}]")
            with c_h2:
                is_edit = st.toggle("✏️ Editar/Borrar", key=f"ed_tog_{row['id']}")

            if not is_edit:
                st.markdown("---")
                v1, v2 = st.columns(2)
                with v1:
                    st.write(f"📞 **Tel:** {row['telefono']}")
                    st.write(f"💼 **Rubro:** {row['rubro']}")
                    st.write(f"🏠 **Resid:** {row['residencia']}")
                    st.write(f"👨‍👩‍👧 **Fam:** {row['grupo_familiar']}")
                with v2:
                    st.write(f"📓 **Contexto:** {row['contexto']}")
                    st.markdown(f"🚀 **Paso:** :orange[{row['proximos_pasos']}]")
            else:
                with st.form(key=f"f_edit_{row['id']}"):
                    f1, f2, f3 = st.columns(3)
                    u_nom = f1.text_input("Nombre", row['nombre'])
                    u_ape = f2.text_input("Apellido", row['apellido'])
                    u_tel = f3.text_input("Teléfono", row['telefono'])
                    
                    u_resp = f1.selectbox("Responsable", LISTA_RESPONSABLES, index=LISTA_RESPONSABLES.index(row['responsable']) if row['responsable'] in LISTA_RESPONSABLES else 0)
                    u_est = f2.selectbox("Estado", ESTADOS, index=ESTADOS.index(row['estado']) if row['estado'] in ESTADOS else 0)
                    u_rub = f3.text_input("Rubro", row['rubro'])
                    
                    u_sug = f1.number_input("Sugerido", value=float(row['monto_sugerido']))
                    u_conf = f2.number_input("Confirmado", value=float(row['monto_confirmado']))
                    u_fam = f3.text_input("Familia", row['grupo_familiar'])
                    
                    u_res = f1.text_input("Residencia", row['residencia'])
                    u_pas = f2.text_input("Próximo Paso", row['proximos_pasos'])
                    u_ctx = st.text_area("Contexto", row['contexto'])
                    
                    if st.form_submit_button("💾 GUARDAR CAMBIOS"):
                        # REGLA: Forzar monto 0 si no es confirmado
                        final_mon = u_conf if u_est == "6. Donación Confirmada" else 0
                        # Actualizar DataFrame usando el índice original de la carga
                        df.loc[idx] = [row['id'], u_nom, u_ape, u_tel, u_rub, u_ctx, u_res, u_fam, u_sug, u_est, final_mon, u_pas, u_resp, row['fecha_registro']]
                        if save_data(df):
                            st.session_state.df = df
                            st.rerun()
                
                if st.button("🗑️ ELIMINAR DONANTE", key=f"del_{row['id']}"):
                    df = df.drop(idx)
                    if save_data(df):
                        st.session_state.df = df
                        st.rerun()

# --- VISTA: NUEVO ---
elif menu == "🆕 Nuevo Registro":
    st.subheader("Cargar Nuevo Prospecto")
    with st.form("n"):
        c1, c2 = st.columns(2)
        n = c1.text_input("Nombre *")
        a = c2.text_input("Apellido")
        r = c1.selectbox("Asignar Responsable", LISTA_RESPONSABLES)
        s = c2.number_input("Monto Sugerido (USD)", value=0.0)
        ctx = st.text_area("Notas de contexto")
        
        if st.form_submit_button("🚀 Crear Donante"):
            if n:
                # ID único basado en timestamp para evitar choques
                new_id = str(int(datetime.now().timestamp()))
                new_row = pd.DataFrame([{
                    "id": new_id, "nombre": n, "apellido": a, "responsable": r, 
                    "monto_sugerido": s, "estado": "1. Por contactar", "monto_confirmado": 0, 
                    "fecha_registro": datetime.now().strftime("%Y-%m-%d"),
                    "telefono": "-", "rubro": "-", "contexto": ctx, "residencia": "-", 
                    "grupo_familiar": "-", "proximos_pasos": "-"
                }])
                df = pd.concat([df, new_row], ignore_index=True)
                if save_data(df):
                    st.session_state.df = df
                    st.success(f"¡{n} añadido correctamente!")
                    st.rerun()
