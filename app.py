                ...
                with st.expander(f"{emoji} {row['nombre']} {row['apellido']} | {row['estado']} | {row['responsable']}"):
                    ...
                    if not is_edit:
                        ...
                        with st.form(key=f"f_edit_{row['id']}"):
                            ...
                            un_sug = f2.number_input("Sugerido", value=float(row['monto_sugerido'])); un_conf = f3.number_input("Confirmado", value=float(row['monto_confirmado']))
                            un_tel = f1.text_input("Teléfono", row['telefono']); un_res = f2.text_input("Residencia", row['residencia'])
                            un_fam = f3.text_input("Familia", row['grupo_familiar']); un_rub = f1.text_input("Rubro", row['rubro'])
                            un_ctx = st.text_area("Contexto", row['contexto']); un_pas = st.text_input("Próximo Paso", row['proximos_pasos'])
                            if st.form_submit_button("💾 GUARDAR CAMBIOS"):
                                target_id = str(row['id'])
                                df.loc[df['id'] == target_id, ['nombre', 'apellido', 'responsable', 'estado', 'monto_sugerido', 'monto_confirmado', 'telefono', 'residencia', 'grupo_familiar', 'rubro', 'contexto', 'proximos_pasos']] = [un_nom, un_ape, u_resp, un_est, un_sug, un_conf, un_tel, un_res, un_fam, un_rub, un_ctx, un_pas]
